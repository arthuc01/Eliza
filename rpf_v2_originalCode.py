...
def calcRPF(ensembles, peakLists, tolerances, distThreshold=5.0, prochiralTolerance=0.04,
    ...
    for peakList in peakLists:
        ...
        if not contexts:
            print
            'Peak list %s has unusable axes' % peakList
            continue
...
def countsToRpf(truePos, falseNeg, truePosNoe, falsePosNoe, verbose=False):
    ...
    if verbose:
        print
        "countsToRpf", falseNeg, truePos, truePosNoe, falsePosNoe
...
def calcDpScore(fMeasure, truePos, falseNeg, truePosNoe, falsePosNoe, verbose=False):
    ...
    if verbose:
        print
        " Free  %.3f, %.3f, %.3f" % (rFree, pFree, fFree)
        print
        " Ideal %.3f, %.3f, %.3f" % (rIdeal, pIdeal, fIdeal)
        print
        " Query %.3f, %.3f, %.3f" % (rQuery, pQuery, fQuery)
        ...
        for peak in peaks:
            ...
            if diagonalTolerance:
                ...
                if peakDim1.value is None:
                    print
                    "Peak %s does not have a peakDim1.value" % peak
                    continue
